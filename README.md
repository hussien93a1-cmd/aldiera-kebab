...
من لوحة التحكم > الإعدادات اضبط:

- إحداثيات المطعم `latitude / longitude`.
- نطاق التوصيل، الافتراضي `7` كم.
- أجور أول 1 كم، الافتراضي `1000` د.ع.
- أجور كل 1 كم إضافي، الافتراضي `500` د.ع.
- طريقة التقريب، والافتراضي `تقريب لأقرب 250 للأعلى`.
- مزود الخرائط.

المزود الافتراضي هو `OSRM` حتى يعمل الاختبار بدون مفتاح API. للإنتاج يفضل اختيار `OpenRouteService` أو `Mapbox` وإضافة API Key من نفس الشاشة. صفحة الزبون لا تستخدم المسافة المستقيمة لحساب الأجرة؛ يجب حساب مسافة الطريق أولًا، ويتم منع الطلب إذا تجاوزت مسافة الطريق نطاق التوصيل.

معادلة التقريب الافتراضية:

```js
Math.ceil(amount / 250) * 250
```

ويتم حفظ `rawDeliveryFee` و `roundedDeliveryFee` و `roundingMethod` داخل الطلب.
